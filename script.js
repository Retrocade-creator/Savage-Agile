...
function startWorkingAgreements() {
    ...
    if (!icebreakerSection || !workingAgreementsSection) {
        console.error('Section elements missing:', { icebreakerSection, workingAgreementsSection });
        alert('Error: Section elements not found. Check console.');
        return;
    }

    icebreakerSection.style.display = 'none';
...
function addAgreement() {
    ...
    if (!input || !agreementList) {
        console.error('Agreement elements missing:', { input, agreementList });
    ...
    if (agreementText !== '') {
        ...
        alert('Please enter a working agreement before adding.');
    }
}

function sortAgreementsByVotes() {
    console.log('sortAgreementsByVotes called');
    const agreementList = document.getElementById('agreement-list');
    if (!agreementList) {
        console.error('Agreement list not found');
        return;
    }

    const items = Array.from(agreementList.getElementsByTagName('li'));
    items.sort((a, b) => {
    ...
    items.forEach(item => agreementList.appendChild(item));
}
...
function prepopulateColumns() {
    console.log('prepopulateColumns called');
    const preset = document.getElementById('column-presets').value;
    const customForm = document.getElementById('custom-columns-form');
    ...
    if (container.children.length > 0) {
        if (!confirm('This will clear existing columns. Continue?')) {
            document.getElementById('column-presets').value = '';
            return;
        }
        container.innerHTML = '';
        columnCount = 0;
...
function createCustomColumns() {
    console.log('createCustomColumns called');
    const container = document.getElementById('columns-container');
    const column1 = document.getElementById('custom-column-1').value.trim();
    const column2 = document.getElementById('custom-column-2').value.trim();
    const column3 = document.getElementById('custom-column-3').value.trim();
    ...
    customColumns.forEach(name => addColumn(name));
    document.getElementById('custom-columns-form').style.display = 'none';
    document.getElementById('column-presets').value = '';
    document.getElementById('custom-column-1').value = '';
    document.getElementById('custom-column-2').value = '';
    document.getElementById('custom-column-3').value = '';
    resetTokens();
}
